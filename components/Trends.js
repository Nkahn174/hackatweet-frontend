...
function Trends({onHashtagClick }){ 
        ...
        useEffect(()=>{
            fetch('https://hackatweet-backend-murex-eta.vercel.app/tweets/trends').then(response=>response.json()).then(
              data=>{
    ...
    const hashtagTrends = trends.map((data,i)=>{
        return(
        <div className={styles.hashtag} onClick={() => onHashtagClick(data.hashtag)} key={i} >
