...
function Hashtag({hashtag }){ 
  ...
  const refreshHashtagTweets = () => {
    if (!hashtag) return;
    fetch(`https://hackatweet-backend-murex-eta.vercel.app//tweets/hashtag/${encodeURIComponent(hashtag)}`)
      .then((res) => res.json())
  ...
  const handleKeyDown = (value) => {
    ...
    fetch(`https://hackatweet-backend-murex-eta.vercel.app/tweets/hashtag/${encodeURIComponent(value)}`)
      .then((res) => res.json())
      .then((data) => setTweets(data.data));
  };
  ...
  const handleTweetDeleted = () => {
    ...
    refreshTrends();         // mettre à jour les tendances
      ...
      return (<Tweet msg={data.message} hastag ={data.hashtags} nbrLike = {data.nbrOfLikes} time ={data.heure} firstname={data.firstname} username={data.username} isOwner={user && user.username === data.username} tweetToken={data.token} key={i} onTweetDeleted={handleTweetDeleted} ></Tweet> )
    })
    return (
