  ...
  const toggleLike = () => {
    ...
    fetch(urlUserLike, {
      ...
      .then((dataUser) => {
        ...
        return fetch(
          "https://hackatweet-backend-murex-eta.vercel.app/tweets/likesupdate",
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  return (
    <div className={styles.tweetContainer}>
      <div className={styles.user}>
        ...
        <p>
          ...
          <span className={styles.tweetInfo}>
            @{props.username} {timeSince(props.time)}{" "}
          </span>{" "}
        </p>
      </div>
      ...
      <div className={styles.tweeInteract}>
        <FontAwesomeIcon
          icon={faHeart}
          style={{ color: isLiked ? "#ff69b4" : "#ffffff" }}
          className={styles.iconToHover}
          onClick={toggleLike}
        />{" "}
        {likesCount}
        {props.isOwner && (
          <FontAwesomeIcon
            ...
            style={{ color: "#ffffff" }}
            className={styles.iconToHover}
            onClick={() => HandleRemoveClick(props.tweetToken)}
          />
        )}
      </div>
    </div>
